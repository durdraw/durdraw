...
class UserInterface():  # Separate view (curses) from this controller
    ...
    def __init__(self, app):
        ...
        self.init_plugins()
        self.statusBar.mainMenu.add_item("Help", self.showHelp, "h", shortcut="esc-h")
        self.statusBar.mainMenu.add_item("Quit", self.safeQuit, "q", shortcut="esc-q")
            ...
            if "type" in plugin["meta"]:
                ...
                if 'menu_item' in plugin["meta"]["type"]:
                    if 'location' in plugin["meta"]:
                        shortcut = ''
                        if 'shortcut' in plugin['meta']:
                            shortcut = plugin["meta"]["shortcut"]
                        ...
                        #self.statusBar.menus[location].move_item_up(itemName)
    ...
    def switchTo256ColorMode(self):
        if self.appState.maxColors < 256:
            ...
            if not self.appState.durview_running:
    ...
    def switchToColorMode(self, newMode: str):
        ...
        if self.appState.sideBarEnabled and not self.playing:
            ...
            if realmaxX > self.mov.sizeX + self.appState.sideBar_minimum_width:
                ...
                #if self.appState.colorMode == "256":
                if not self.appState.fetchMode and not self.appState.durview_running:
                    self.statusBar.colorPicker.show()
                #if self.appState.colorMode == "16":
